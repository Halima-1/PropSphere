...
export const useDeleteProperty = () => {
    ...
    return useCallback(
        async (
            ...
            const loading = toast.loading("getingt Property...");

            try {
                await contract.removeProperty.staticCall(
                    _id
                );
                const estimatedGas = await contract.removeProperty.estimateGas(
                    _id
                );

                const estimatedGasPlusBuffer = (120n * estimatedGas) / 100n;

                const tx = await contract.removeProperty(
                    _id,
                    {
